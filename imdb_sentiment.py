...
# DEBUG
train_dataset, valid_dataset = random_split(list(train_dataset)[0:2500], [2000, 500])
...
def encode_transform_batch(device, tokenizer, model, hidden_size):
    ...
    def collate_fn(batch):
        ...
        return all_data_tensor.to(device), label_list.to(device)
...
class MLP(nn.Module):
    def __init__(self, input_dim, fc_hidden_size, loss_fn):
        super().__init__()
...
loss_fn = nn.BCELoss()
model = MLP(hidden_size, 1024, loss_fn)
